    ...
    public class QuoteExpiryJob
    {
        ...
        public async Task RunAsync(CancellationToken ct)
        {
            ...
            var expiredItems = await (
                ...
                join est in _db.cost_estimates on q.estimate_id equals est.estimate_id
                where q.status == "Sent"
                ...
                select new ExpiredItem
                {
                    ...
                    EstimateCreatedAt = est.created_at,
            ...
            _logger.LogInformation("[QuoteExpiryJob] Found {count} expired quote items", expiredItems.Count);

            var groups = expiredItems
            ...
            foreach (var g in groups)
            {
                ...
                // Update tất cả quote hết hạn trong request này
                var quoteIds = g.Select(x => x.QuoteId).Distinct().ToList();

                var quotes = await _db.quotes
                    .Where(q => quoteIds.Contains(q.quote_id))
                    .ToListAsync(ct);
                ...
                if (!string.IsNullOrWhiteSpace(consultantEmail))
                {
                    try
                    {
                        ...
                        var html = BuildConsultantExpiredEmailHtml(req, g.ToList(), reason, now);
                        await _emailService.SendAsync(consultantEmail, subject, html);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogWarning(ex, "[QuoteExpiryJob] Failed to send consultant mail for request_id={id}", requestId);
                    }
                }
            }

            await _db.SaveChangesAsync(ct);
            _logger.LogInformation("[QuoteExpiryJob] Done. Updated {count} requests", groups.Count);
        }
        ...
        private static string BuildConsultantExpiredEmailHtml(
            dynamic req,
            List<ExpiredItem> expiredList,
            string reason,
            DateTime now)
        {
            // Liệt kê bảng các báo giá hết hạn
            var rows = string.Join("", expiredList
                .OrderBy(x => x.ExpiredAt)
                .Select(x => $@"
<tr>
  <td style='padding:8px;border:1px solid #e2e8f0'>#{x.QuoteId}</td>
  <td style='padding:8px;border:1px solid #e2e8f0'>#{x.EstimateId}</td>
  <td style='padding:8px;border:1px solid #e2e8f0'>{x.EstimateCreatedAt:dd/MM/yyyy HH:mm:ss}</td>
  <td style='padding:8px;border:1px solid #e2e8f0'>{x.ExpiredAt:dd/MM/yyyy HH:mm:ss}</td>
</tr>"));
...
<body style='font-family: Arial, sans-serif; background:#f1f5f9; padding: 24px;'>
  <div style='max-width:760px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.08)'>
    ...
    <div style='padding:22px'>
      ...
      <div style='margin-top:18px;font-weight:800'>Danh sách báo giá hết hạn (theo estimate.created_at + 24h)</div>
      <table style='width:100%;border-collapse:collapse;margin-top:10px;font-size:13px'>
        <thead>
          <tr style='background:#f8fafc'>
            <th style='padding:8px;border:1px solid #e2e8f0;text-align:left'>Quote ID</th>
            <th style='padding:8px;border:1px solid #e2e8f0;text-align:left'>Estimate ID</th>
            <th style='padding:8px;border:1px solid #e2e8f0;text-align:left'>Estimate CreatedAt</th>
            <th style='padding:8px;border:1px solid #e2e8f0;text-align:left'>ExpiredAt</th>
          </tr>
        </thead>
        <tbody>
          {rows}
    ...
    <div style='background:#f8fafc;padding:14px;text-align:center;color:#94a3b8;font-size:12px;border-top:1px solid #e2e8f0'>
      Email tự động từ hệ thống MES
        ...
        private class ExpiredItem
        {
            ...
            public DateTime EstimateCreatedAt { get; set; }
